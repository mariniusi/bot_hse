...
mistake_text = 'Не совсем понимаю, что ты имеешь в виду. Введи /start'

task = ''

#вводим самого бота
bot = telebot.TeleBot('6801292770:AAGIVLoAyTnN7v8nEQKGN0hzlPhIRqK9sGU')
...
def user(message):
    ...
    bot.send_photo(message.chat.id, get('https://hi-news.ru/wp-content/uploads/2020/12/balaeniceps_bird_image_three-750x423.jpg').content)
...
#блок 6
def pogoda(message):
    bot.send_photo(message.chat.id, get('https://farm6.static.flickr.com/5308/5842242575_2d46c4f667_b.jpg').content)
...
@bot.message_handler(commands=['text'])
def summa(message):
    bot.send_photo(message.chat.id, get('https://pbs.twimg.com/media/FJVkpWbacAAzfr3.jpg:large').content)
    bot.send_message(message.from_user.id, 'Введите сумму в той валюте, которую хотите перевести 🤑')
...
def step_on2(message):
    ...
    if choice =='главное меню':
        ...
        text += '' +'\n'
        text += 'Я умею:'+'\n'
        text += '🍄 Показывать погоду в выбранном городе' +'\n'
        text += '(но если ты находишься на территории России, ' +'\n'
        text += 'для корректной работы тебе ' +'\n'
        text += 'надо включить ВПН)' +'\n'
        text += ''+'\n'
        ...
        text += '' +'\n'
        text += '🍄 Напоминать о важных задачах ' +'\n'
        text += 'в установленное тобой время'
...
def dead(message):
        ...
        knop.add(but1, but2, but3, but4)
        bot.send_photo(message.chat.id, get('https://cdn.fishki.net/upload/post/2020/09/19/3424665/tn/foto-2.jpg').content)
